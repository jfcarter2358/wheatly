    ...
    for object_name in statement['objects']:
        ...
        for action_name in statement['actions']:
            ...
            try:
                ...
                logger.ERROR(f'No method found for {object_name} with name action_{action_name}')
                full_success = False
                continue
            print(context)
            # run the action function
            context, success = function(context, logger, statement['args'], statement['modifiers'])
            if not success:
                full_success = False
